...
Action[ACTION_CONST_MAGE_FROST] = {
    ...
    HighVoltage         = Action.Create({ Type = "Spell", ID = 461248, isTalent = true}),
    ...
    UnerringProficiency = Action.Create({ Type = "Spell", ID = 444974, isTalent = true}),
...
A[3] = function(icon)
    ...
    local fightRemains      = MultiUnits:GetByRangeAreaTTD(40, 10)
    ...
    function EnemyRotation(unitID)
        ...
        for Unit_Nameplates in pairs(MultiUnits:GetActiveUnitPlates()) do
            ...
            if Unit_Nameplates 
                ...
                and not notInterruptable  -- Verifica se o alvo não é não-interrompível English please xD cant understand that
            then
        ...
        local function BurstRotation()
            ...
            if A.ArcaneSurge:IsReady(player) and A.TouchOfTheMagi:GetCooldown() < (A.ArcaneSurge:GetExecuteTime() + GetCurrentGCD()) then
                return A.ArcaneSurge:Show(icon)
            end

            -- Radiant Spark if talented
            if A.RadiantSpark:IsReady(unitID) and A.RadiantSpark:IsTalentLearned() then
        ...
        local function CommonRotation()
            ...
            -- Nether Tempest (falls gelernt)
            if A.NetherTempest:IsReady(unitID) and A.NetherTempest:IsTalentLearned() and Unit(unitID):HasDeBuffs(A.NetherTempest.ID, true) <= 3 then
                return A.NetherTempest:Show(icon)
            end

            -- Build and Spend Logik für Arcane Charges
        ...
        local function NormalSingleTarget()
            if CommonRotation() then return true end
        ...
        local function NormalAoE()
            if CommonRotation() then return true end

            -- AoE-Logic
            if A.ArcaneExplosion:IsReady(player) and MultiUnits:GetActiveUnitPlates() >= 3 then
        ...
        local function SpellSlinger()
            if MultiUnits:GetByRange(40) <= 1 and A.SpellSlinger:IsTalentLearned() then
                ...
                if A.ShiftingPower:IsReady(player) and (
                    (Unit(player):HasBuffs(A.ArcaneSurge.ID) == 0 and Unit(player):HasBuffs(A.SiphonStorm.ID) == 0 and 
                     Unit(target):HasDeBuffs(A.TouchOfTheMagi.ID) == 0 and A.Evocation:GetCooldown() > 15 and 
                     A.TouchOfTheMagi:GetCooldown() > 10 and A.ArcaneOrb:GetCooldown() > 0 and A.ArcaneOrb:GetCharges() == 0) or
                    (Player:PrevGCD(1, A.ArcaneBarrage) and (Unit(player):HasBuffs(A.ArcaneSurge.ID) > 0 or Unit(target):HasDeBuffs(A.TouchOfTheMagi.ID) > 0 or A.Evocation:GetCooldown() < 20))
                ) then
        ...
        if isBurst and BurstRotation() then
            return true
        end

        if A.SpellSlinger:IsTalentLearned() then
            if MultiUnits:GetActiveEnemies() >= 2 then
                if SpellSlingerAOE() then return true end
            else
                if SpellSlinger() then return true end
            end
        elseif A.SunFury:IsTalentLearned() then
            if MultiUnits:GetActiveEnemies() >= 2 then
                if SunFuryAOE() then return true end
            else
                if SunFury() then return true end
            end
        else
        -- Fallback to normal Rotation
            if MultiUnits:GetActiveEnemies() >= 2 then
                if NormalAoE() then return true end
            else
                if NormalSingleTarget() then return true end
            end
        end

        -- Fallback general against errors and stuck Icons
        if A.ArcaneBlast:IsReady(unitID) then
            return A.ArcaneBlast:Show(icon)
        end
    end
