...
A[3] = function(icon, isMulti)
    ...
    local MissingSpiritBomb = Unit("target"):HasDeBuffs(A.SpiritBombDebuff.ID, true) == 0
    local Fierydebuffed = Unit("target"):HasDeBuffs(A.FieryBrandDebuff.ID, true) == 0
    ...
    local function EnemyRotation(unit)    
        ...
        local function CovenantCall()
            ...
            --actions.cooldown+=/elysian_decree
            if A.ElysianDecree:IsReady(player) and Player:IsStayingTime() > 0.2 and Unit(unit):GetRange() <= 5 and A.RazelikhsDefilement:HasLegendaryCraftingPower() then
                return A.ElysianDecree:Show(icon)
            end                
        ...
        local function DamageRotation()
            ...
            if A.SoulCleave:IsReady(unit) and Player:Fury() >= 30 and (SoulFragments <= 2 and A.SpiritBomb:IsTalentLearned()) and not A.FelDevastation:IsReady("player") then
                return A.SoulCleave:Show(icon)
            end
            
            --Spirit Bomb target no debuff
            if A.SpiritBomb:IsReady(unit) and A.SpiritBomb:IsTalentLearned() and MissingSpiritBomb then
            ...
            if A.FelDevastation:IsReady("player") and Unit("target"):GetRange() <= 15 and Player:Fury() >= 50 and FelDevDMG then
                return A.FelDevastation:Show(icon)
            end
            
            --Soul Cleave Spirit Bomb Build Fury Safe for FelDev
            if A.FelDevastation:IsReady("player") and A.SoulCleave:IsReady(unit) and Player:Fury() >= 50 and (SoulFragments <= 2 and A.SpiritBomb:IsTalentLearned()) then
                return A.SoulCleave:Show(icon)
            end
            
            --Soul Cleave Fiery Brand and other Builds
            if A.SoulCleave:IsReady(unit) and Player:Fury() >= 30 and (SoulFragments >= 2 and not A.SpiritBomb:IsTalentLearned()) then
                return A.SoulCleave:Show(icon)
            end
            
            --Sigil of Flame (try not to overlap with Sigil from Abyssal Strike talent)
            if A.SigilofFlame:IsReady("player") and not (A.AbyssalStrike:IsSpellLearned() and A.InfernalStrike:GetSpellTimeSinceLastCast() < 4) and not Unit(player):InVehicle() and not A.RazelikhsDefilement:HasLegendaryCraftingPower() and Unit("target"):GetRange() <= 10 then
        ...
        local function DefenseRotation()
            ...
            if A.DemonSpikes:IsReady("player") and Unit("player"):HasBuffs(A.DemonSpikesBuff.ID, true) == 0 and Fierydebuffed and A.DemonSpikes:GetSpellCharges() > 1.9 and Unit("player"):HasBuffs(A.Metamorphosis.ID, true) == 0 and (A.LastPlayerCastID ~= A.DemonSpikes.ID) and Unit(player):HealthPercent() <= DemonSpikes1HP then
                return A.DemonSpikes:Show(icon)
            end
            
            if A.DemonSpikes:IsReady("player") and Unit("player"):HasBuffs(A.DemonSpikesBuff.ID, true) == 0 and Fierydebuffed and A.DemonSpikes:GetSpellCharges() >= 1 and Unit("player"):HasBuffs(A.Metamorphosis.ID, true) == 0 and (A.LastPlayerCastID ~= A.DemonSpikes.ID) and Unit(player):HealthPercent() <= DemonSpikes2HP then
                return A.DemonSpikes:Show(icon)
            end            
            ...
            --Fel Devastation on cooldown
            if A.FelDevastation:IsReady("player") and Unit("player"):HasBuffs(A.Metamorphosis.ID, true) == 0 and Unit("target"):GetRange() <= 15 and Unit(player):HealthPercent() <= FelDevHP then
                return A.FelDevastation:Show(icon)
            end            
