...
A[3] = function(icon)
    ...
    function EnemyRotation(unitID)
        ...
        local function BurstRotation()
            ...
            if A.Berserking:IsReady(player) and (
                (Player:PrevGCD(1, A.ArcaneSurge.ID) and opener == 1) or
                (Player:PrevGCD(1, A.ArcaneSurge.ID) and (fightRemains < 80 or Unit("target"):HealthPercent() < 35 or not A.ArcaneBombardment:IsTalentLearned() or Unit(player):HasBuffs(A.SpymastersWebBuff.ID) > 0)) or
                (Player:PrevGCD(1, A.ArcaneSurge.ID) and not A.SpymastersWeb:IsEquipped())) then
                return A.Berserking:Show(icon)
            end
            if A.Fireblood:IsReady(player) and (
                (Player:PrevGCD(1, A.ArcaneSurge.ID) and opener == 1) or
                (Player:PrevGCD(1, A.ArcaneSurge.ID) and (fightRemains < 80 or Unit("target"):HealthPercent() < 35 or not A.ArcaneBombardment:IsTalentLearned() or Unit(player):HasBuffs(A.SpymastersWebBuff.ID) > 0)) or
                (Player:PrevGCD(1, A.ArcaneSurge.ID) and not A.SpymastersWeb:IsEquipped())) then
                    return A.Fireblood:Show(icon)
            end
            if A.AncestralCall:IsReady(player) and (
            (Player:PrevGCD(1, A.ArcaneSurge.ID) and opener == 1) or
            (Player:PrevGCD(1, A.ArcaneSurge.ID) and (fightRemains < 80 or Unit("target"):HealthPercent() < 35 or not A.ArcaneBombardment:IsTalentLearned() or Unit(player):HasBuffs(A.SpymastersWebBuff.ID) > 0)) or
            (Player:PrevGCD(1, A.ArcaneSurge.ID) and not A.SpymastersWeb:IsEquipped())) then
            ...
            if A.SpymastersWeb:IsReady(player) and (
            Player:PrevGCD(1, A.ArcaneSurge.ID) or Player:PrevGCD(1, A.Evocation.ID) or 
            (Unit(player):HasBuffs(A.EtherealPowerlink.ID) > 0 and Unit(player):HasBuffs(A.SiphonStorm.ID) > 0 and A.SpellfireSpheres:IsTalentLearned())) 
            and (fightRemains < 80 or Unit("target"):HealthPercent() < 35 or not A.ArcaneBombardment:IsTalentLearned() or (Unit(player):HasBuffsStacks(A.SpymastersReport.ID) == 40 and fightRemains > 240)) or fightRemains < 20 then
                return A.SpymastersWeb:Show(icon)
            end
        ...
        if isBurst then
            local Burst = BurstRotation()
            if Burst then
                return Burst:Show(icon)
            end
        end
...
A[6] = function(icon)
    if UnitExists("raid1") or UnitExists("party1") then
        return PartyRotation(icon, UnitExists("raid1") and "raid1" or "party1")
    end
end

A[7] = function(icon)
    if UnitExists("raid2") or UnitExists("party2") then
        return PartyRotation(icon, UnitExists("raid2") and "raid2" or "party2")
    end
end

A[8] = function(icon)
    if UnitExists("raid3") or UnitExists("party3") then
        return PartyRotation(icon, UnitExists("raid3") and "raid3" or "party3")
    end
end
